...
static void CheckError(uint32_t flags, const stacktype &original_stack,
    ...
    uint32_t consensusBranchId = NetworkUpgradeInfo[Consensus::UPGRADE_BUTTERCUP].nBranchId;
    bool r = EvalScript(stack, script, flags, sigchecker, consensusBranchId, &err);
    BOOST_CHECK(!r);
...
BOOST_AUTO_TEST_CASE(checkdatasig_test) {
    ...
    for (uint32_t flags = 0; flags < (1U << 11); flags++) {
